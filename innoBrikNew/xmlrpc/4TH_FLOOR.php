...
$xml = simplexml_load_file("nodeDataXML.xml");
...
in an alphabetic order.';
	...
	function loginResponse($m)
	{
		...
		if ($pass->scalarval() == "mpxadmin" && $usr->scalarval() == "mpxadmin"){
			return new xmlrpcresp(new xmlrpcval("sessionIdSample"));
			//return new xmlrpcresp($xml);
		}
		else {
			$err = "User Name and password are invalid";
			return new xmlrpcresp(0, $xmlrpcerruser, $err);
		}
	}
	function invokeResponse($m)
	{
		$dom = new DomDocument();
		$dom->load("4TH/broadway.xml");
		
		$xp = new domxpath($dom);

		
		$sessionId = $m->getParam(0);
		$str1 = $m->getParam(1);
		$identi = $m->getParam(2);
		if ($identi->scalarval() == "children_names" || $identi->scalarval() == "has_child")
		{
			$URLstr = explode("/",$str1->scalarval());
			$URL = "/mpxconfig/node";
			foreach ($URLstr as $node){
				//echo "$node";
				if ($node != "")
				{
					$URL = $URL ."/node[@name = '" .$node ."']";
			...
			foreach ($titles as $node) {
				//echo $node->textContent . " ";
				//echo $node->getAttributeNode('name')->textContent ." ";
				array_push($a,$node->getAttributeNode('name')->textContent);
				
			}
			if ($identi->scalarval() == "children_names")
			{
				
				$res = join(",",$a); 
				return new xmlrpcresp(new xmlrpcval($res));
			}
			else if ($identi->scalarval() == "has_child")
			{
				$childName = $m->getParam(3);
				foreach ($titles as $node) {
					//echo $node->textContent . " ";
					//echo $node->getAttributeNode('name')->textContent ." ";
					if ($node->getAttributeNode('name')->textContent == $childName->scalarval())
					{
						$strExist = True;
					}
				}
				return new xmlrpcresp(new xmlrpcval($strExist));
			}
			
		}
		
	}
	function getchildrenBroadway($m)
	{
...
';
	...
	$a=array(
		"rna_xmlrpc2.create_session" => array (
			"function" => "loginResponse",
			"singature" => $findstate_sig,
			"docstring" => $findstate_doc
		),
		"rna_xmlrpc2.invoke" => array (
